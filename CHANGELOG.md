...
- **Breaking change**: When adding or updating cards, this module no longer uses an `exactPattern` and `prefixPattern` model. Instead, it takes an array of patterns. See <https://github.com/braintree/credit-card-type##pattern-detection> for details.
...
- **Breaking change**: Mastercard enum changed from `master-card` to `mastercard`
